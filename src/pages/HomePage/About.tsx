        ...
        <div className="relative w-[35%]">
          <div className="relative  bg-[#EDE8DC]">
            ...
            <div className="absolute -bottom-10 -right-6  bg-[#005F40] text-white px-8 py-6 shadow-xl">
                <p className="text-4xl font-bold text-[#D4AF37] leading-none">35+</p>
                <p>{t("about.statYears")}</p>
            </div>
          </div>
        </div>
       <div className="w-[50%]">
        <h6 className="text-[#D4AF37] font-montserrat text-xs font-semibold tracking-[0.25em] uppercase mb-3">{t("about.established")}</h6>
        <h1 className="text-3xl lg:text-4xl font-playfair text-[#1C2B3A] mb-6 font-normal leading-snug">{t("about.titleStart")} <span className="italic">{t("about.titleItalic")}</span></h1>
        <div className="w-20 h-0.5 bg-[#d4af37]"></div>
        <p className="text-[#4A5568] leading-relaxed mb-5 text-[15px]">{t("about.p1")}</p>
        <br />
        <p className="text-[#4A5568] leading-relaxed mb-5 text-[15px]">{t("about.p2")}</p>
       </div>
      </div>
    </div>
  );
